"""Centralized logging setup for Arba.
...
# Used by _ComponentFilter and exposed for ``arba logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Arba logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the Arba home directory.  Falls back to
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the arba group. However, both ``_open()`` (initial creation)

...
description: "Operate the Teams meeting summary pipeline via Arba CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
author: Arba + Teknium
license: MIT
prerequisites:
...
Everything operator-facing is a `arba teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
```bash
arba teams-pipeline validate              # config snapshot — run first after any change
arba teams-pipeline token-health          # Graph token status
arba teams-pipeline token-health --force-refresh   # force a fresh token acquisition
arba teams-pipeline list                  # recent meeting jobs
arba teams-pipeline list --status failed  # only failed jobs
arba teams-pipeline show <job-id>         # full detail of one job
arba teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
arba teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
arba teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
arba teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
arba teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

arba teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
arba teams-pipeline delete-subscription <sub-id>
arba teams-pipeline maintain-subscriptions            # renew near-expiry ones
arba teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `arba teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `arba cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).

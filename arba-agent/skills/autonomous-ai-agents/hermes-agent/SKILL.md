---
name: hermes-agent
description: "Configure, extend, or contribute to Arba."
version: 2.1.0
author: Arba + Teknium
license: MIT
...
- **Self-improving through skills** — Arba learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
- **Persistent memory across sessions** — remembers who you are, your preferences, environment details, and lessons learned. Pluggable memory backends (built-in, Honcho, Mem0, and more) let you choose how memory works.
- **Multi-platform gateway** — the same agent runs on Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Email, and 10+ other platforms with full tool access, not just chat.
- **Provider-agnostic** — swap models and providers mid-workflow without changing anything else. Credential pools rotate across multiple API keys automatically.
- **Profiles** — run multiple independent Arba instances with isolated configs, sessions, skills, and memory.
- **Extensible** — plugins, MCP servers, custom tools, webhook triggers, cron scheduling, and the full Python ecosystem.

People use Arba for software development, research, system administration, data analysis, content creation, home automation, and anything else that benefits from an AI agent with persistent context and full system access.
...
# Single query
arba chat -q "What is the capital of France?"
...
```
arba setup [section]      Interactive wizard (model|terminal|gateway|tools|agent)
arba model                Interactive model/provider picker
arba config               View current config
arba config edit          Open config.yaml in $EDITOR
arba config set KEY VAL   Set a config value
arba config path          Print config.yaml path
arba config env-path      Print .env path
arba config check         Check for missing/outdated config
arba config migrate       Update config with new options
arba auth                 Interactive credential manager
arba auth add PROVIDER    Add OAuth or API-key credential (e.g. nous, openai-codex, qwen-oauth)
arba auth list            List stored credentials
arba auth remove PROVIDER Remove a stored credential
arba doctor [--fix]       Check dependencies and config
arba status [--all]       Show component status
```

### Tools & Skills

```
arba tools                Interactive tool enable/disable (curses UI)
arba tools list           Show all tools and status
arba tools enable NAME    Enable a toolset
arba tools disable NAME   Disable a toolset

arba skills list          List installed skills
arba skills search QUERY  Search the skills hub
arba skills install ID    Install a skill (ID can be a hub identifier OR a direct https://…/SKILL.md URL; pass --name to override when frontmatter has no name)
arba skills inspect ID    Preview without installing
arba skills config        Enable/disable skills per platform
arba skills check         Check for updates
arba skills update        Update outdated skills
arba skills uninstall N   Remove a hub skill
arba skills publish PATH  Publish to registry
arba skills browse        Browse all available skills
arba skills tap add REPO  Add a GitHub repo as skill source
```

### MCP Servers

```
arba mcp serve            Run Arba as an MCP server
arba mcp add NAME         Add an MCP server (--url or --command)
arba mcp remove NAME      Remove an MCP server
arba mcp list             List configured servers
arba mcp test NAME        Test connection
arba mcp configure NAME   Toggle tool selection
```

### Gateway (Messaging Platforms)

```
arba gateway run          Start gateway foreground
arba gateway install      Install as background service
arba gateway start/stop   Control the service
arba gateway restart      Restart the service
arba gateway status       Check status
arba gateway setup        Configure platforms
```
...
```
arba sessions list        List recent sessions
arba sessions browse      Interactive picker
arba sessions export OUT  Export to JSONL
arba sessions rename ID T Rename a session
arba sessions delete ID   Delete a session
arba sessions prune       Clean up old sessions (--older-than N days)
arba sessions stats       Session store statistics
```

### Cron Jobs

```
arba cron list            List jobs (--all for disabled)
arba cron create SCHED    Create: '30m', 'every 2h', '0 9 * * *'
arba cron edit ID         Edit schedule, prompt, delivery
arba cron pause/resume ID Control job state
arba cron run ID          Trigger on next tick
arba cron remove ID       Delete a job
arba cron status          Scheduler status
```

### Webhooks

```
arba webhook subscribe N  Create route at /webhooks/<name>
arba webhook list         List subscriptions
arba webhook remove NAME  Remove a subscription
arba webhook test NAME    Send a test POST
```

### Profiles

```
arba profile list         List all profiles
arba profile create NAME  Create (--clone, --clone-all, --clone-from)
arba profile use NAME     Set sticky default
arba profile delete NAME  Delete a profile
arba profile show NAME    Show details
arba profile alias NAME   Manage wrapper scripts
arba profile rename A B   Rename a profile
arba profile export NAME  Export to tar.gz
arba profile import FILE  Import from archive
```

### Credential Pools

```
arba auth add             Interactive credential wizard
arba auth list [PROVIDER] List pooled credentials
arba auth remove P INDEX  Remove by provider + index
arba auth reset PROVIDER  Clear exhaustion status
```

### Other

```
arba insights [--days N]  Usage analytics
arba update               Update to latest version
arba pairing list/approve/revoke  DM authorization
arba plugins list/install/remove  Plugin management
arba honcho setup/status  Honcho memory integration (requires honcho plugin)
arba memory setup/status/off  Memory provider config
arba completion bash|zsh  Shell completions
arba acp                  ACP server (IDE integration)
arba claw migrate         Migrate from OpenClaw
...
/snapshot [sub]      Create or restore state snapshots of Arba config/state (CLI)
/background <prompt> Run prompt in background
/queue <prompt>      Queue for next turn
/steer <prompt>      Inject a message after the next tool call without interrupting
/agents (/tasks)     Show active agents and running tasks
/resume [name]       Resume a named session
/goal [text|sub]     Set a standing goal Arba works on across turns until achieved
...
/busy [sub]          Control what Enter does while Arba is working (CLI)
...
/update              Update Arba to latest (gateway)
...
Edit with `arba config edit` or `arba config set section.key value`.
...
20+ providers supported. Set via `arba model` or `arba setup`.

| Provider | Auth | Key env var |
|----------|------|-------------|
| OpenRouter | API key | `OPENROUTER_API_KEY` |
| Anthropic | API key | `ANTHROPIC_API_KEY` |
| Nous Portal | OAuth | `arba auth` |
| OpenAI Codex | OAuth | `arba auth` |
...
| Qwen OAuth | OAuth | `arba auth add qwen-oauth` |
...
Enable/disable via `arba tools` (interactive) or `arba tools enable/disable NAME`.
...
Common "why is Arba doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `hermes` invocation) because they're read once at startup.
...
```bash
arba config set security.redact_secrets true       # keep enabled globally
```
...
```bash
arba config set security.redact_secrets false
```
...
```bash
arba config set privacy.redact_pii true    # enable
arba config set privacy.redact_pii false   # disable (default)
```

### Command approval prompts

By default (`approvals.mode: manual`), Arba prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
```bash
arba config set approvals.mode smart       # recommended middle ground
arba config set approvals.mode off         # bypass everything (not recommended)
```
...
To keep the model away from network or media tools entirely, open `arba tools` and toggle per-platform. Takes effect on next session (`/reset`). See the Tools & Skills section above.
...
## Spawning Additional Arba Instances

Run additional Arba processes as fully independent subprocesses — separate sessions, tools, and environments.
...
```
terminal(command="arba chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# Background for long tasks:
terminal(command="arba chat -q 'Set up CI/CD for ~/myapp'", background=true)
```

### Interactive PTY Mode (via tmux)

Arba uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
```
# Agent A: backend
terminal(command="tmux new-session -d -s backend -x 120 -y 40 'arba -w'", timeout=10)
terminal(command="sleep 8 && tmux send-keys -t backend 'Build REST API for user management' Enter", timeout=15)

# Agent B: frontend
terminal(command="tmux new-session -d -s frontend -x 120 -y 40 'arba -w'", timeout=10)
...
terminal(command="tmux new-session -d -s resumed 'arba --continue'", timeout=10)

# Resume specific session
terminal(command="tmux new-session -d -s resumed 'arba --resume 20260225_143052_a1b2c3'", timeout=10)
```
...
- **Use `arba chat -q` for fire-and-forget** — no PTY needed
...
the `cronjob` tool, the `arba cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `arba curator <verb>` — `status`, `run`, `pause`, `resume`,
...
Users drive it via `arba kanban <verb>`; dispatcher-spawned workers
...
Arba runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
without BOM. `arba config edit` writes without BOM; manual edits in
...
Arba's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Arba-installed venv at
...
every Arba tool and most Windows APIs. Prefer forward slashes in code
...
### Tool not available
1. `arba tools` — check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `arba doctor` — check config and dependencies
2. `arba auth` — re-authenticate OAuth providers (or `arba auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `arba model` → GitHub Copilot.
...
### Skills not showing
1. `arba skills list` — verify installed
2. `arba skills config` — check platform enablement
3. Load explicitly: `/skill name` or `arba -s name`
...
```bash
arba config set auxiliary.vision.provider <your_provider>
arba config set auxiliary.vision.model <model_name>
```
...
**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The Arba-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:

/**
 * ChatPage — embeds `arba --tui` inside the dashboard.
 *
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__HERMES_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `arba dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Arba TUI has enabled xterm mouse-events
    ...
    // forwarding them into Arba. Keyboard input, paste, and resize still
    // behave normally.

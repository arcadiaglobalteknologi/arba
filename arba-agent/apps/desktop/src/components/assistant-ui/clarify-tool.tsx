...
export const ClarifyTool = (props: ToolCallMessagePartProps) => {
  ...
  // Once Arba records an answer, fall back to the standard tool block so
...
function ClarifyToolPending({ args }: ToolCallMessagePartProps) {
  ...
  const respond = useCallback(
    async (answer: string) => {
      ...
      if (!gateway) {
        notifyError(new Error('Arba gateway is not connected'), 'Could not send clarify response')

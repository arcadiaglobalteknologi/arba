...
export function GatewaySettings() {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      ...
      notify({
        ...
        message: apply ? 'Arba Desktop will reconnect using the saved settings.' : 'Saved for the next restart.'
      })
    } catch (err) {
  ...
  const testRemote = async () => {
    ...
    try {
      ...
      const message = `Connected to ${result.baseUrl}${result.version ? ` · Arba ${result.version}` : ''}`
      setLastTest(message)
    ...
    <SettingsContent>
      <div className="mb-5">
        ...
        <p className="mt-2 max-w-2xl text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-(--ui-text-tertiary)">
          Arba Desktop starts its own local gateway by default. Use a remote gateway when you want this app to control
          an already-running Arba backend on another machine or behind a trusted proxy.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <ModeCard
          ...
          description="Start a private Arba backend on localhost. This is the default and works offline."
        ...
        <ModeCard
          ...
          description="Connect this desktop shell to a remote Arba backend using its session token."
          disabled={state.envOverride}
          icon={Globe}

...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: 'Update Arba', onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
    message: 'Your Arba backend is older than this desktop build and may not work correctly. Update to align them.',
...
function onFocus() {
  ...
  // updated Arba from another window/CLI between focuses, and About should

 ...
 *
 * Stamps the Arba icon + identity onto the packed Windows Arba.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `arba desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Arba')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Arba'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Arba.exe keeps the stock Electron icon`)
  }
}

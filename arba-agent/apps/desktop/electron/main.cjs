...
// HERMES_HOME — the user-facing root for everything Arba-related. Mirrors
...
// %LOCALAPPDATA%\arba yet, prefer the legacy path so we don't orphan their
...
// ACTIVE_HERMES_ROOT — the canonical mutable Arba install. Same path
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Arcadia Global Teknologi'
})
...
let bootProgressState = {
  ...
  message: 'Waiting to start Arba backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function findSystemPython() {
  ...
  // CPython but several Arba deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Arba' terminal tool requires
...
// ─── Self-update (git-pull against the running backend's arba root) ──────
...
// ALL repo mutation — running `arba update` + rebuilding the desktop — so
...
// `arba update`). Instead we spawn the staged Arba-Setup binary with
// --update and quit, so it can run `arba update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // whole update itself: `arba update` (backend) + `arba desktop
    ...
    if (!updater) {
      ...
      // `arba desktop`, never the Tauri installer that self-copies
      ...
      // checkout they're on — bare `arba update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'arba update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `arba update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `arba update` if branch detection fails.
      }
    ...
    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Arba updater…', percent: 100 })

    // Detached so the updater outlives this process — it needs us GONE before
    // `arba update` will run (the venv shim is locked while we live).
...
// Resolve the arba CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `arba` (or legacy `hermes`) on PATH.
function resolveHermesCliBinary(updateRoot) {
  const venvArba = path.join(updateRoot, 'venv', 'bin', 'arba')
  if (fileExists(venvArba)) return venvArba
  const venvHermes = path.join(updateRoot, 'venv', 'bin', 'hermes')
  if (fileExists(venvHermes)) return venvHermes
  return findOnPath('arba') || findOnPath('hermes') || null
}
...
// macOS/Linux in-app update: backend (`arba update`) + OS-aware GUI rebuild
// (`arba desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts = {}) {
  ...
  if (!hermes) {
    emitUpdateProgress({ stage: 'manual', message: 'arba update', percent: null })
    return { ok: true, manual: true, command: 'arba update', hermesRoot: updateRoot }
  }

  // Put the Arba-managed Node and the venv on PATH so `arba desktop`'s
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Arba (git + dependencies)…', percent: 10 })
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'arba update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'arba update failed' }
  }
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Arba to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Arba.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Arba.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Arba to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Arba to load the new version.',
      percent: 100
    })
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Arba.app/Contents/...`
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Arba at ${ACTIVE_HERMES_ROOT}`,
...
function resolveHermesBackend(dashboardArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Arba source at ${overrideRoot}`, dashboardArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Arba source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }
  ...
  //    (applyUpdates -> git pull) or `arba update` from the CLI.
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Arba override under WSL: ${hermesOverride}`)
      }
    } else {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Arba CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return {
          label: `existing Arba CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Arba CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'Arba not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Arba install found; starting first-launch bootstrap')
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Arba install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Arba bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `Arba install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Arba' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Arba on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Arba venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Arba at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Arba runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Arba backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Arba backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Arba backend after ${timeoutMs}ms`))
    })
...
async function waitForHermes(baseUrl, token) {
  ...
  throw new Error(`Arba backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function resolveRemoteBackend() {
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Arba backend.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Arba gateway is selected, but no session token is saved. ' +
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Arba backend', 8)
    const remote = resolveRemoteBackend()
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Arba backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Arba backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Arba runtime', 28)
    const backend = await ensureRuntime(resolveHermesBackend(dashboardArgs))
    const hermesCwd = resolveHermesCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Arba backend via ${backend.label}`, 84)
    rememberLog(`Starting Arba backend via ${backend.label}`)
    ...
    hermesProcess.once('error', error => {
      rememberLog(`Arba backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Arba backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      rememberLog(`Arba backend exited (${signal || code})`)
      hermesProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Arba backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Arba backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })
    ...
    await advanceBootProgress('backend.wait', 'Waiting for Arba backend to become ready', 90)
    await Promise.race([waitForHermes(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Arba backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Arba' TUI into
...
ipcMain.handle('hermes:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Arba.')
  }
...
// Resolve the canonical Arba version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Arba version instead of the Electron app's own package.json version,
...
function maybePinToDock() {
  ...
  // file:///Applications/Arba.app/ -- NOT a raw POSIX path. A type-0/raw-path

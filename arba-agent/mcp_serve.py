"""
Arba MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Arba-specific extra)

Usage:
    arba mcp serve
    arba mcp serve --verbose
...
class EventBridge:
    ...
    This is the Arba equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Arba MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "Arba messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Arba MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

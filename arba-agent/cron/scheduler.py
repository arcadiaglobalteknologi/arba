...
# Without this, standalone invocations (e.g. after `arba update` reloads
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``arba tools`` config for the ``cron`` platform.
...
def _get_hermes_home() -> Path:
    """Resolve Arba home dynamically while preserving test monkeypatch hooks."""
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Arba profile.
    ...
    the scheduler's test/override hook and a context-local Arba home override
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # Arba home override, scheduler _hermes_home hook, and temporary

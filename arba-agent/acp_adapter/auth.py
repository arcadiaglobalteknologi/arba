"""ACP auth helpers — detect and advertise Arba authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Arba runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Arba can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Arba.
    ...
    not have Arba provider credentials configured yet, so Arba always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Arba using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Arba provider",
            description=(
                "Open Arba' interactive model/provider setup in a terminal. "
                "Use this when Arba has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )

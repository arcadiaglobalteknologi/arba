"""ACP session manager — maps ACP sessions to Arba AIAgent instances.
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Arba itself is running in WSL.

    Windows ACP clients can launch ``arba acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Arba from a Windows workspace while the ACP process runs
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Arba agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Arba AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Arba runtime provider configuration.

...
There are two ways to add a platform to the Arba gateway:
...
**zero changes to core Arba code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `arba gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Arba core.

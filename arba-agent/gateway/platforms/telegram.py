...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Arba-created private-chat topic lanes are marked with
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `arba update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Arba or OpenClaw instance is running "
            "with this token, then restart the gateway with 'arba gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Arba process (shared mount, same machine, etc.).
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``arba model`` picker via the shared ``group_providers`` fold,
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Arba bot profiles. A message like

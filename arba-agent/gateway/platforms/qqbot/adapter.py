...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Arba upgrades overwrite this
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          ...
          detached ``arba update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``arba update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `arba setup gateway` / `arba gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``arba update --gateway`` watcher. Button

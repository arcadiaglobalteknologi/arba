...
In single-bot mode (what Arba currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Arba gateway is already using this Feishu app_id"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Arba does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            response = await client.get(
                file_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; ArbaAgent/1.0)",
                    "Accept": "*/*",
                },
            )
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Arba webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Arba' SessionSource fields.
...
# Called by `arba gateway setup` via _setup_feishu() in hermes_cli/gateway.py.
# =============================================================================

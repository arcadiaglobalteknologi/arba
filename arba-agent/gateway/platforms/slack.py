...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # manifest (see `arba slack manifest`). In Socket Mode, Slack
            ...
            if _slash_names:
                _slash_pattern = _re.compile(
                    r"^/(?:" + "|".join(_re.escape(n) for n in _slash_names) + r"|hermes)$"
                )  # legacy /hermes alias kept for backward compat
            else:  # pragma: no cover - registry always non-empty
                _slash_pattern = _re.compile(r"^/(?:arba|hermes)$")
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Arba handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Arba session — matching the per-thread behavior channels already

...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Arba Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Arba/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Arba Desktop. If it serves Arba/Desktop UI or another unrelated app, stop that process and report failure.",
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Arba provider is configured.",
                },
            )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`arba setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`arba gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`arba sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`arba config edit` needs $EDITOR in a real terminal"
    return None

...
the actual `setup.sh` that creates the project workspace, configures Arba
...
> [arcadiaglobalteknologi/kanban-video-pipeline](https://github.com/arcadiaglobalteknologi/kanban-video-pipeline).
...
2. **Create profiles** — `arba profile create <name> --clone`
...
6. **Fire the initial kanban task** — `arba kanban create` assigned to the director
...
```bash
arba profile create director --clone 2>/dev/null || true
```
...
PyYAML must be installed in the user's Python (it ships with most Arba
...
```bash
arba kanban create "Direct production of <video title>" \
    --assignee director \

...
Maps each role archetype to the Arba skills it should `always_load` and the
...
## Arba skills relevant to video production
...
These are **not** Arba skills but external CLIs / APIs that profiles invoke.
...
## Built-in Arba tools for media review

These are native Arba tools — not invoked via terminal but through their own
...
| Anthropic | `ANTHROPIC_API_KEY` | every Arba profile (Claude) |
...
When a new Arba-public video skill ships:

...
description: Migrate a user's OpenClaw customization footprint into Arba. Imports Arba-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Arba (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [Migration, OpenClaw, Arba, Memory, Persona, Import]
    related_skills: [hermes-agent]
---

# OpenClaw -> Arba Migration

Use this skill when a user wants to move their OpenClaw setup into Arba with minimal manual cleanup.
...
```bash
arba claw migrate              # Full interactive migration
...
arba claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `arba setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Arba home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Arba memory entries
- merge OpenClaw command approval patterns into Arba `command_allowlist`
- migrate Arba-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Arba working-directory configuration
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Arba workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Arba destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Arba-compatible secrets, currently:
...
Arba CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Arba. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Arba workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Arba skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Arba unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Arba targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Arba destination. Unsupported auth blobs must still be reported as skipped.
...
- Arba persona state imported
- Arba memory files populated with converted OpenClaw knowledge

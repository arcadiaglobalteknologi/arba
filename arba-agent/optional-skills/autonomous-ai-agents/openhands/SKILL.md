...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Arba
license: MIT
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Arba.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Arba-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hermes-agent` (Arba subagents via `delegate_task`).

...
version: 0.1.0
author: Matt Maximo (MattMaximo), Arba
license: MIT
...
# Grok Build CLI — Arba Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Arba terminal. Grok
...
1. Prepare stable input files first with Arba tools (`read_file`,
...
2. **Don't conflate Arba' xAI auth with the `grok` CLI's auth.** Arba'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Arba' xAI auth carries
   over.

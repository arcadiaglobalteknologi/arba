---
name: honcho
description: Configure and use Honcho memory with Arba -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Arba
license: MIT
...
# Honcho Memory for Arba

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Arba profile its own peer identity while sharing a unified view of the user.
...
```bash
arba honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Arba profile's AI peer
...
Honcho models conversations as interactions between **peers**. Arba creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Arba instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Arba run |
| `global` | Single session across all directories |

Manual override: `arba honcho map my-project-name`
...
Each Arba profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
arba profile create coder --clone
...
```bash
arba honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Arba when Honcho memory is active.
...
Run `arba honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### Memory not persisting across sessions
Check `arba honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `arba profile create <name> --clone`. For existing profiles: `arba honcho sync`.
...
| Command | Description |
|---------|-------------|
| `arba honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `arba honcho status` | Show resolved config, connection test, peer info for active profile |
| `arba honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `arba honcho disable` | Disable Honcho for the active profile |
| `arba honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `arba honcho peers` | Show peer identities across all profiles |
| `arba honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `arba honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `arba honcho sessions` | List known directory-to-session-name mappings |
| `arba honcho map <name>` | Map current working directory to a Honcho session name |
| `arba honcho identity` | Seed AI peer identity or show both peer representations |
| `arba honcho sync` | Create host blocks for all Arba profiles that don't have one yet |
| `arba honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Arba + Honcho |
| `arba memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `arba memory status` | Show active memory provider and config |
| `arba memory off` | Disable external memory provider |

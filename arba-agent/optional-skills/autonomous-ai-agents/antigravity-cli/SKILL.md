...
version: 0.1.0
author: Tony Simons (asimons81), Arba
license: MIT
...
commands through the Arba `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from Arba itself.

...
> **Bundled with Arba.** Originally authored by [@PCinkusz](https://github.com/PCinkusz) at https://github.com/PCinkusz/hermes-achievements — vendored into `plugins/hermes-achievements/` so it ships with the dashboard out-of-the-box and stays in lockstep with Arba feature changes. Upstream repo remains the staging ground for new badges and UI iteration.
>
> When Arba is installed via `pip install hermes-agent` or cloned from source, this plugin auto-registers as a dashboard tab on first `arba dashboard` launch. No separate install step. See [Built-in Plugins → hermes-achievements](../../website/docs/user-guide/features/built-in-plugins.md) in the main docs.

Achievement system for the Arba Dashboard: collectible, tiered badges generated from real local Arba session history.

![Arba Achievements dashboard](docs/assets/achievements-dashboard-hd.png)

The screenshots use temporary demo tier data to show the full visual range. The plugin itself reads real local Arba session history by default.
...
Arba Achievements scans local Arba sessions and unlocks badges based on real agent behavior:

- autonomous tool chains
- debugging and recovery patterns
- vibe-coding file edits
- Arba-native skills, memory, cron, and plugin usage
...
- **Secret** — hidden until Arba detects the first related signal
...
Clone into your Arba plugins directory:
...
If backend API routes 404, restart `arba dashboard`; plugin APIs are mounted at dashboard startup.
...
If the update changes backend routes or `plugin_api.py`, restart `arba dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing Arba session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.

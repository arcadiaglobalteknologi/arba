"""SimpleX Chat platform adapter (Arba plugin).
...
This adapter ships as a Arba platform plugin under
``plugins/platforms/simplex/``. The Arba plugin loader scans the
...
discoverable and `arba setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Arba — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``arba setup gateway`` → SimpleX.
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `arba gateway status` without

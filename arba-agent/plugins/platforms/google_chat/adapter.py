...
# ``model_tools`` import time, so ``arba status``, ``arba chat``, etc.
...
#     fresh "Arba is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # they don't sit in the chat forever as "Arba is thinking…".
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Arba is thinking…' marker message.
        ...
        "Arba is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Arba is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Arba is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Arba is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Arba is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``arba setup``.
    ...
    print_info("Restart the gateway: arba gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``arba cron`` running as a
    separate process from ``arba gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Arba plugin system at startup.
    ...
    ctx.register_platform(
        ...
        # the "Arba is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Arba is thinking…' marker message "

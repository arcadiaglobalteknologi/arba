"""
IRC Platform Adapter for Arba.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Arba agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Arba")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Arba shutting down")
...
def check_requirements() -> bool:
    ...
    # arba setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `arba gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Arba to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    print_info("Restart the gateway for changes to take effect: arba gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Arba (cron)")

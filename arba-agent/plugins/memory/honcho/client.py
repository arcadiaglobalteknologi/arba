...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Arba instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Arba profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Arba profile.
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Arba system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Arba profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Arba session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Arba session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Arba session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'arba honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run arba honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `arba honcho setup` to configure)."
        )

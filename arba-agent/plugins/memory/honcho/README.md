...
```bash
arba memory setup honcho   # configure Honcho directly (works on a fresh install)
arba memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
arba config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hermes/.env
```

> `arba honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `arba memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated Arba instances) |
| 2 | `~/.hermes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Arba profile: `hermes` (default) or `hermes_<profile>`.
...
**Deployment shapes** (`arba memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Arba to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Arba session ID (`20260415_a3f2b1`) |
...
- **`per-directory`** — basename of `$PWD`. Opening arba in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Arba session ID (timestamp + hex). Every `hermes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Arba profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Host key is derived from the active Arba profile: `hermes` (default) or `hermes_<profile>` (e.g. `arba -p coder` -> host key `hermes_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `arba memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `arba honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `arba memory setup`) |
| `arba honcho status` | Show resolved config for active profile |
| `arba honcho enable` / `disable` | Toggle Honcho for active profile |
| `arba honcho mode <mode>` | Change recall or observation mode |
| `arba honcho peer --user <name>` | Update user peer name |
| `arba honcho peer --ai <name>` | Update AI peer name |
| `arba honcho tokens --context <N>` | Set context token budget |
| `arba honcho tokens --dialectic <N>` | Set dialectic max chars |
| `arba honcho map <name>` | Map current directory to a session name |
| `arba honcho sync` | Create host blocks for all Arba profiles |

...
```bash
arba memory setup    # select "supermemory"
```

Or manually:

```bash
arba config set memory.provider supermemory
...
Use `{identity}` in the `container_tag` to scope memories per Arba profile:

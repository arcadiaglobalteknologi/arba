...
``{title, url, description, position}`` rows every other Arba web
provider produces.
...
prefers Arba-managed xAI Grok OAuth (via ``arba auth``) and falls back
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``arba tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `arba auth` to sign in with "

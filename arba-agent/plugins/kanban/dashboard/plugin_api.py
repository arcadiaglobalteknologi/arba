...
This means ``arba dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``arba kanban
...
def list_diagnostics(
    ...
    AND for ``arba kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``arba kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``arba kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``arba kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def _active_profile_name() -> str:
    """Return the current Arba profile name for notify-sub ownership."""
    try:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``arba profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``arba kanban decompose <task_id>``.

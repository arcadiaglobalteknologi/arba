...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:ARBA_HOME) { $env:ARBA_HOME } elseif ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\arba" }),
    [string]$InstallDir = $(if ($env:ARBA_HOME) { "$env:ARBA_HOME\arba" } elseif ($env:HERMES_HOME) { "$env:HERMES_HOME\arba" } else { "$env:LOCALAPPDATA\arba\arba" }),
    ...
    # builds apps/desktop into a launchable Arba.exe.
    #
    # Why opt-in:
    #   * Arba-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Arba.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Arba.exe
    ...
    #     `arba desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:arcadiaglobalteknologi/arba.git"
$RepoUrlHttps = "https://github.com/arcadiaglobalteknologi/arba.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Install-Uv {
    # Arba owns its own uv at $HermesHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `arba update` stay in sync.
...
function Install-Git {
    ...
    which Arba uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\arba\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Arba needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Arba copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\arba\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HERMES_GIT_BASH_PATH`` (User scope) so Arba can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Arba features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Arba needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so Arba can find it even before
    ...
    Write-Warn "Could not locate bash.exe -- Arba may not find Git Bash."
...
function Test-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (Arba-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\arba-node-extract"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\arba-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running arba processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/arcadiaglobalteknologi/arba/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/arcadiaglobalteknologi/arba/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/arcadiaglobalteknologi/arba/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\arba-$zipLabel.zip"
                $extractPath = "$env:TEMP\arba-extract"
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install arba package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # users hit and lazy-import errors from `arba dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `arba dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `arba dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up arba command..."
    ...
    # Add the venv Scripts dir to user PATH so arba is globally available
    ...
    # Only needed on Windows where we install to %LOCALAPPDATA%\arba instead
    ...
    Write-Success "arba command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.hermes-bootstrap-complete which tells the Arba
    ...
    $pinnedBranch = $Branch
    if (-not $pinnedBranch) {
        $pinnedBranch = "master"  # install.ps1's own default for -Branch
    }
...
function Copy-ConfigTemplates {
    ...
    # (the default PS5 behaviour), and Arba's prompt-injection scanner
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Arba-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'arba setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\arba-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\arba-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\arba-npm-tui-$(Get-Random).log"
        [void](_Run-NpmInstall "TUI" $tuiDir $tuiLog $npmExe)
    }
}

function Install-Desktop {
    # Build apps/desktop into a launchable Arba.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/Arba.exe directly,
    ...
    # The --dir mode produces an unpacked Arba.exe in
    ...
    # are NOT trying to work around). The Arba icon + product name are
    # stamped onto Arba.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\arba-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Arba.exe",
        "$desktopDir\release\win-arm64-unpacked\Arba.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Arba.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Arba icon + identity are stamped onto Arba.exe by the
    ...
    #    Arba.exe. We deliberately do NOT point them at `arba desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Arba.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Arba.lnk')
        )
        ...
        # with the Arba icon, but without this the shortcut can keep drawing
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'arba setup'."
        return
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\arba.exe"
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'arba whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: arba gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: arba gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: arba gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "   arba setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   arba config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   arba config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   arba gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   arba update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Arba repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Arba-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Arba to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }

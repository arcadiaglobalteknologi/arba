...
# Bootstrap Open WebUI against Arba's OpenAI-compatible API server.
#
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-Arba}"
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Arba
...
main() {
  ...
  log 'Ensuring Arba API server is configured...'
  ...
  log 'Restarting Arba gateway so API server settings take effect...'
  arba gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'Arba API server did not answer on the first check. Trying to start gateway in the background...'
    nohup arba gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "Arba API endpoint: ${HERMES_API_BASE_URL}"

...
# xAI's public API is OpenAI-compatible for the endpoints Arba commonly
# uses. The Responses endpoint is included because Arba' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Arba-managed OAuth credentials."""

    auth_hint = "arba auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`arba auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`arba auth reset xai-oauth` or re-authenticate with "
                    "`arba auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `arba auth add xai-oauth --type oauth`."
            )

"""CLI handlers for the ``arba proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "arba proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"arba auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Arba proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Arba proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: arba proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``arba proxy <subcommand>``."""
    ...
    print(
        "arba proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  arba proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  arba proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  arba proxy providers\n"

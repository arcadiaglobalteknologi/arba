"""
MCP Server Management CLI — ``arba mcp`` subcommand.

Implements ``arba mcp add/remove/list/test/configure`` for interactive
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``arba mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  arba mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  arba mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  arba mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: arba mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `arba mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  arba mcp add <name> --url <endpoint>')
        _info('  arba mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `arba mcp remove` + `arba mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `arba mcp login " + name + "`.")
            return
        if tools:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'arba mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``arba mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `arba plugin`.
        from hermes_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("arba mcp                                    Open the catalog picker (default)")
        _info("arba mcp catalog                            List Nous-approved MCPs")
        _info("arba mcp install <name>                     Install a catalog MCP")
        _info("arba mcp serve                              Run as MCP server")
        _info("arba mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("arba mcp add <name> --command <cmd>         Add a stdio server")
        _info("arba mcp add <name> --preset <preset>       Add from a known preset")
        _info("arba mcp remove <name>                      Remove a server")
        _info("arba mcp list                               List configured servers")
        _info("arba mcp test <name>                        Test connection")
        _info("arba mcp configure <name>                   Toggle tools")
        _info("arba mcp login <name>                       Re-authenticate OAuth")
        print()

...
``arba gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``arba gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every Arba runtime call inside the
    container drops to arba via ``s6-setuidgid``, that silent failure
...
# active (set via the ``arba -p <profile>`` flag before the call). This
...
#
# Per-profile gateways are registered dynamically when `arba profile create`
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the arba user.
    ...
    (the ``hermes`` user — UID 10000 — running every Arba runtime
    ...
    is dispatched as the arba user.
    ...
    with arba ownership before triggering ``s6-svscanctl -a``,
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the arba user already — directory is hermes-
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``arba -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`arba profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the arba user and exec's
             ``arba -p <profile> gateway run`` (or just ``hermes
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``arba gateway
        ...
        if profile == "default":
            gateway_cmd = "arba gateway run"
        else:
            gateway_cmd = f"arba -p {shlex.quote(profile)} gateway run"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``arba logs`` reads and what
        ...
        return (
            ...
            f'exec s6-setuidgid arba s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with arba ownership
            ...
            # the arba user) won't hit EACCES on root-owned 0700

...
Arba owns its own uv binary at ``$HERMES_HOME/bin/uv`` (or ``uv.exe`` on
...
def managed_uv_path() -> Path:
    """Return the path where Arba keeps *its* uv binary.
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``arba update`` so the managed copy stays current.

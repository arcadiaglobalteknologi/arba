"""``arba logs`` — view and filter Arba log files.
...
Usage examples::
    ...
    arba logs                    # last 50 lines of agent.log
    arba logs -f                 # follow agent.log in real time
    arba logs errors             # last 50 lines of errors.log
    arba logs gateway -n 100    # last 100 lines of gateway.log
    arba logs gui -f            # follow gui.log (dashboard/pty/ws)
    arba logs --level WARNING    # only WARNING+ lines
    arba logs --session abc123   # filter by session ID substring
    arba logs --component tools  # only tool-related lines
    arba logs --since 1h         # lines from the last hour
    arba logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Arba runs — try 'arba chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'arba chat' to generate logs)")

"""`arba checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hermes/checkpoints/``.  Actions:

    arba checkpoints               # same as `status`
    arba checkpoints status        # total size, project count, breakdown
    arba checkpoints list          # per-project checkpoint counts + workdir
    arba checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    arba checkpoints clear [-f]    # nuke the entire base (asks first)
    arba checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    arba checkpoints
    arba checkpoints prune --retention-days 3 --max-size-mb 200
    arba checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: arba checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``arba checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `arba checkpoints` → status

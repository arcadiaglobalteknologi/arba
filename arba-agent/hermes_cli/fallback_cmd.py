"""
arba fallback — manage the fallback provider chain.
...
Subcommands:
  arba fallback [list]   Show the current fallback chain (default when no subcommand)
  arba fallback add      Pick provider + model via the same picker as `arba model`,
                           then append the selection to the chain
  arba fallback remove   Pick an entry to delete from the chain
  arba fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.arba/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `arba model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  arba fallback add")
        print()
        return
    ...
    print("  Docs: https://arcadiaglobalteknologi.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `arba model`, then append the selection to the chain."""
    ...
    print("  `arba model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `arba fallback list` to view, or `arba fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``arba fallback [subcommand]``."""

...
def remove_path_from_shell_configs():
    """Remove Arba PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Arba" comment and following line
                # (legacy "# Arba Agent" markers are still matched for backward compat)
                if '# Arba' in line or '# Arba Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    """Remove the arba wrapper script if it exists."""
    wrapper_paths = [
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    when the link still resolves into this Arba home's ``node`` directory.
    A link the user has since repointed at nvm (or anything else outside
    Arba) is left untouched so we never break unrelated tooling.
    """
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``arba gateway run`` processes
...
#   4. On the ``arba dashboard`` + gateway paths, drops files into
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Arba-owned User-PATH entries."""
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Arba-owned entries from User-scope PATH in the registry.
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``arba -p <name> gateway stop|uninstall`` because
...
def run_uninstall(args):
    ...
    print(color("│                   ⚕ Arba Uninstaller                   │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Arba data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Arba code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Arba-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Arba-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing arba command...")
    ...
    #     (only when they still point into this Arba home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Arba-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Arba-managed node/npm/npx symlinks found")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/arcadiaglobalteknologi/arba/master/arba-agent/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/arcadiaglobalteknologi/arba/master/arba-agent/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Arba! ⚕")
    print()

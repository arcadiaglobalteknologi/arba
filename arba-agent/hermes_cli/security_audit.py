"""On-demand supply-chain audit for Arba installs.

Scans three surfaces a Arba user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Arba venv (every PyPI dist via ``importlib.metadata``).
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `arba security audit`."""

...
Handles: arba gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `arba update`.
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``arba gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``arba gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    patterns = [
        ...
        "arba gateway",
        "arba gateway",  # legacy command alias
        ...
        # ``arba.exe --profile`` / ``arba.exe -p`` would also match
        # ``arba.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "arba.exe gateway",
        "hermes.exe gateway",  # legacy command alias
        "arba-gateway.exe",
        "hermes-gateway.exe",
        "gateway/run.py",
    ]
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``arba update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Arba profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Arba profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``arba update`` users
    # with no running gateway until they re-invoke ``arba gateway``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}arba gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}arba gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `arba gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``arba gateway status`` output so users with
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Arba Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``arba gateway run`` must remain interruptible from
...
SERVICE_DESCRIPTION = "Arba Gateway - Messaging Platform Integration"
...
# Legacy service names from older Arba installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Arba gateway units.

    Detects unit files installed by older Arba versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Arba gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Arba gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Arba gateway unit(s) detected from an older install:")
    ...
    print_info("    arba gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Arba gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Arba gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: arba gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo arba gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    arba gateway uninstall")
    print_info("    sudo arba gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Arba can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo arba gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo arba gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo arba gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Arba often sets ``HOME`` to a profile-scoped directory, but
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``arba update`` later relocates/removes — the path
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Arba install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo arba gateway uninstall --system")
    print_info("         arba gateway install")
    print_info("         arba gateway start")
...
def systemd_install(
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}arba gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}arba gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}arba gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `arba gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `arba gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}arba gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}arba gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}arba gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}arba gateway start{scope_flag}"
        )
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Arba install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  arba gateway status             # Check status")
...
def launchd_stop():
    ...
    # `arba gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Arba install")
    else:
        print("⚠ Service definition is stale relative to the current Arba install")
        print("  Run: arba gateway start")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Arba gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Arba command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `arba gateway run` still
    ...
    # /restart) rather than through `arba gateway restart` which already
    ...
    # `arba gateway start/restart` — leaving the gateway vulnerable to
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Arba will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Arba delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Arba agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Arba will use (e.g., hermes@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Arba connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: arba pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Arba will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``arba setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run arba under
        WSL.
    """
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'arba pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: arba pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'arba gateway setup'"
                    )
            continue
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: arba pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'arba gateway setup'")
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Arba will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Arba will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `arba gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `arba pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `arba pairing approve`."
        )
    elif access_idx == 1:
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: arba gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: arba gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: arba gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo arba gateway install --system"
                        )
                    print_info("  Or run in foreground:  arba gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: arba gateway run")
                print_info(
                    "  For persistence:   tmux new -s hermes 'arba gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: arba gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup arba gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'arba gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``arba gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``arba gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: arba gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: arba gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'arba gateway run'"
                )
                print()
            ...
            print()
            print(
                "  arba gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s hermes 'arba gateway run'         # persistent via tmux"
            )
            print(
                "  nohup arba gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  arba profile create <name>     # creates the s6 service slot")
                print("  arba -p <name> gateway start   # bring it up via s6")
                print("  arba status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: arba gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: arba gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: arba gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  arba profile delete <name>     # tears down the s6 service slot")
                print("  arba -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `arba -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: arba gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `arba gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `arba gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: arba gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  arba gateway install")
                else:
                    print("To install as a service:")
                    print("  arba gateway install")
                    print("  sudo arba gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  arba gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup arba gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'arba gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  arba gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  arba gateway install  # Install as user service")
                    print(
                        "  sudo arba gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Arba gateway unit files from

"""MCP picker — interactive `arba mcp picker` (also the default `arba mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``arba mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `arba plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Arba session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `arba mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: arba mcp install <name>    Picker: arba mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Arba is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Arba — run `arba update` "
...
def show_catalog() -> None:
    """`arba mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`arba mcp picker` (and default `arba mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`arba mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `arba mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:

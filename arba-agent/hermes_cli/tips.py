...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or arba -c.",
    ...
    "Ctrl+Z suspends Arba to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "arba -c resumes your most recent CLI session. arba -c \"project name\" resumes by title.",
    "arba -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "arba -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "arba chat -t web,terminal enables only specific toolsets for a focused session.",
    "arba chat -s github-pr-workflow preloads a skill at launch.",
    "arba chat -q \"query\" runs a single non-interactive query and exits.",
    "arba chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "arba chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "arba --yolo bypasses all dangerous command approval prompts for the entire session.",
    "arba chat --source telegram tags the session for filtering in arba sessions list.",
    "arba -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "arba doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "arba dump outputs a compact setup summary — great for bug reports.",
    "arba config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "arba config edit opens config.yaml in your default editor.",
    "arba config check scans for missing or stale configuration options.",
    "arba sessions browse opens an interactive session picker with search.",
    "arba sessions stats shows session counts by platform and database size.",
    "arba sessions prune --older-than 30 cleans up old sessions.",
    "arba skills search react --source skills-sh searches the skills.sh public directory.",
    "arba skills check scans installed hub skills for upstream updates.",
    "arba skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "arba skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "arba mcp add github --command npx adds MCP servers from the command line.",
    "arba mcp serve runs Arba itself as an MCP server for other agents.",
    "arba auth add lets you add multiple API keys for credential pool rotation.",
    "arba completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "arba logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "arba backup creates a zip backup of your entire Arba home directory.",
    "arba profile create coder creates an isolated profile that becomes its own command.",
    "arba profile create work --clone copies your current config and keys to a new profile.",
    "arba update syncs new bundled skills to ALL profiles automatically.",
    "arba gateway install sets up Arba as a system service (systemd/launchd).",
    "arba memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "arba webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: arba tools disables unused tools, arba skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "arba models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as arba -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Arba tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'arba profile create coder' creates the 'coder' command.",
    "arba profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Arba prints a resume command with session ID and stats.",
    "arba sessions export backup.jsonl exports all sessions for backup or analysis.",
    "arba -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "arba skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: arba cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Arba runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "arba gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Arba via a local macOS server.",
    ...
    "Arba loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "arba mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "arba mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "arba mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — arba tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.arba/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "arba chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "arba plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "arba acp runs Arba as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "arba auth add nous or arba auth add openai-codex sets up OAuth-based providers.",
    ...
    "arba status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.arba/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Arba your own.",
    ...
    "Before a gateway session reset, Arba auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.arba/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.arba/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'arba profile create ls' would be rejected.",
    "arba profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "arba claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via arba tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.arba/.env.",
    "Skills from trusted repos (Arcadia Global Teknologi) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.arba/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Arba auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Arba config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Arba is working.',
    ...
    '/reload reloads ~/.arba/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.arba/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.arba/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'arba curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "arba curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'arba curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'arba auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'arba dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'arba dashboard --tui embeds the full Arba TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.arba/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.arba/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.arba/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.arba/skills/ so drop-in skills appear without restarting the session.',
    ...
    'arba -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'arba chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'arba chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'arba chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "arba chat --source tool tags programmatic chats so they don't clutter arba sessions list.",
    'arba dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'arba sessions rename <ID> "new title" renames any past session; arba sessions delete <ID> removes one.',
    'arba import restores a session export or profile archive produced by sessions export or profile export.',
    'arba fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'arba pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'arba setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'arba status --deep runs the full health sweep across every component; plain arba status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.arba/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Arba block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from arba sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.arba/dashboard-plugins/.',
]

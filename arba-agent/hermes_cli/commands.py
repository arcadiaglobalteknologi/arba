"""Slash command definitions and autocomplete for the Arba CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Arba config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Arba works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Arba is working", "Configuration",
    ...
    CommandDef("update", "Update Arba to the latest version", "Info"),
...
Telegram only displays a small BotCommand menu in practice.  The full Arba
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``arba skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``arba skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``arba skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("arba", "Talk to Arba or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")

"""
Profile management for multiple isolated Arba instances.
...
Usage::
    ...
    arba profile create coder          # fresh profile + bundled skills
    arba profile create coder --clone  # also copy config, .env, SOUL.md, skills
    arba profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    arba -p coder chat                 # or via flag
    arba profile use coder             # set as sticky default
    arba profile delete coder          # remove profile + alias + service
"""
...
# Marker file written by `arba profile create --no-skills`.  When present in
...
# `arba skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Arba infrastructure directories that only the default profile
...
_RESERVED_NAMES = frozenset({
    "arba", "hermes", "default", "test", "tmp", "root", "sudo",
})

# Arba subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the arba root, NOT to the current HERMES_HOME
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Arba installation itself or a common system binary.  "
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, arba subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a arba subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``arba profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Arba config
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``arba update`` skips re-seeding this profile's
    ...
    # Write the opt-out marker so seed_profile_skills() and `arba update`'s
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `arba profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `arba -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``arba profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            ...
            "To remove everything, use: arba uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: arba profile create {canon}"
        )
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: arba profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            ...
            "Specify a different name: arba profile import <archive> --name <name>"
        )
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any arba modules

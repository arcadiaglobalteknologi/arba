"""
Configuration management for Arba.
...
- arba config          - Show current configuration
- arba config edit     - Open config in editor
- arba config set      - Set a specific value
- arba config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``arba logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  arba config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Arba makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Arba itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``arba update``, the TUI build.
...
#   on every plugin install / ``arba update``.
...
#   ``HERMES_ENV`` — Arba runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Arba runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Arba runtime location "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `arba plugins enable
    # observability/langfuse` or `arba tools → Langfuse`); credentials gate
...
def is_managed() -> bool:
    """Check if Arba is running in package-manager-managed mode.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Arba was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
    ...
    container detection here:
      ...
      - the published ``arcadiaglobalteknologi/arba`` image stamps ``docker``
    ...
    so ``arba update`` bailed with "doesn't apply inside the Docker
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Arba lives in a ``uv tool`` layout.
    ...
    the *active* Arba is a regular pip/venv install, causing
    ``arba update`` to upgrade the wrong copy. It would also block on a
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade hermes-agent"
    if method == "docker":
        return "docker pull arcadiaglobalteknologi/arba:latest"
    if method == "pip":
...
# Long-form text for ``arba update`` / ``--check`` when running inside the
...
#     — that script installs a *new* host-side Arba, it doesn't update
...
✗ ``arba update`` doesn't apply inside the Docker container.

Arba runs as a published image (arcadiaglobalteknologi/arba), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull arcadiaglobalteknologi/arba:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate arba
...
Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    ...
    tags at https://hub.docker.com/r/arcadiaglobalteknologi/arba/tags
  • Your config and session history live under ``$ARBA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``arba update`` inside Docker.
...
def format_managed_message(action: str = "modify this Arba installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Arba installation is managed by NixOS "
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Arba installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
    ...
    return (
        f"Cannot {action}: this Arba installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Arba."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Arba commonly set these to map the in-container
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the arba group can
...
def _is_container() -> bool:
    ...
    When Arba runs in a container with volume-mounted config files, forcing
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Arba-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Arba
    ...
    "terminal": {
        ...
        # skipped. When empty, Arba auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Arba sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Arba
        ...
        # bundled Arba image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Arba sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Arba should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``arba chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: arba sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Arba truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``arba kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``arba kanban decompose`` and the kanban
        ...
        # ``arba profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `arba model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hermes` (and `arba chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `arba --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `arba -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `arba --tui` drops a one-time hint
    ...
    # response. If not, Arba feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Arba auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `arba curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `arba update`, so pruning them only
        ...
        # user can roll back with `arba curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Arba*" header.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `arba doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Arba to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `arba -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # decomposition is manual via `arba kanban decompose <id>` or
    ...
    #
    # Core Arba tools (terminal, read_file, write_file, patch,
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://arcadiaglobalteknologi.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `arba model` invocation; network failures
    ...
    "gateway": {
        ...
        # behavior: files must live under the Arba cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Arba
    ...
    "sessions": {
        ...
        # default of ``arba sessions prune``.
        "retention_days": 90,
    ...
    # ``arba update`` behaviour.
    "updates": {
        # Run a full ``arba backup``-style zip of HERMES_HOME before every
        # ``arba update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``arba import <path>``.  Off by default —
    ...
    # `arba tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'arba model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Arba auth store)",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Arba API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Arba API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Arba API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `arba update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Arba versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'arba setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Arba won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'arba doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `arba plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `arba curator status` has no
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: arba config set <key> <value>")
...
#   openai-codex (OAuth — arba auth) — OpenAI Codex
#   nous         (OAuth — arba auth) — Nous Portal
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`arba tools`, `arba setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `arba tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Arba env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to Arba — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Arba vars that are no longer in .env
...
def show_config():
    ...
    print(color("│                  ⚕ Arba Configuration                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  arba config edit     # Edit config file", Colors.DIM))
    print(color("  arba config set <key> <value>", Colors.DIM))
    print(color("  arba setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: arba config set <key> <value>")
            print()
            print("Examples:")
            print("  arba config set model anthropic/claude-sonnet-4")
            print("  arba config set terminal.backend docker")
            print("  arba config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'arba config migrate' to add them")
        ...
        print("  arba config           Show current configuration")
        print("  arba config edit      Open config in editor")
        print("  arba config set <key> <value>   Set a config value")
        print("  arba config check     Check for missing/outdated config")
        print("  arba config migrate   Update config with new options")
        print("  arba config path      Show config file path")
        print("  arba config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``arba config`` UI so users

...
Pure logic: walks a Arba config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `arba doctor` and a future `arba migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Arba config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Arba config and return retirement issues.

"""CLI subcommand: ``arba send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"arba send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"arba send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"arba send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"arba send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"arba send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `arba gateway setup`, or run the gateway once so")
...
def _load_hermes_env() -> None:
    ...
    startup that ``arba send`` must replicate when invoked standalone:
    ...
    intentionally reimplement the minimum needed here so ``arba send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if not target:
        print(
            "arba send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  arba send --to telegram \"hello\"\n"
            "  arba send --to discord:#ops --file report.md\n"
            "  arba send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "arba send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `arba send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Arba "
        ...
        epilog=(
            "Examples:\n"
            "  arba send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | arba send --to telegram:-1001234567890\n"
            "  arba send --to discord:#ops --file /tmp/report.md\n"
            "  arba send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  arba send --list                  # all platforms\n"
            "  arba send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `arba send --list telegram`.",
    )

"""arba hooks — inspect and manage shell-script hooks.

Usage::

    arba hooks list
    arba hooks test <event> [--for-tool X] [--payload-file F]
    arba hooks revoke <command>
    arba hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``arba hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: arba hooks {list|test|revoke|doctor}")
        print("Run 'arba hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `arba hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `arba hooks test` and `arba hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Arba wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `arba hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `arba hooks doctor` would execute

#!/usr/bin/env python3
"""
Arba CLI - Main entry point.

Usage:
    hermes                     # Interactive chat (default)
    arba chat                # Interactive chat
    arba gateway             # Run gateway in foreground
    arba gateway start       # Start gateway as service
    arba gateway stop        # Stop gateway service
    arba gateway status      # Show gateway status
    arba gateway install     # Install gateway service
    arba gateway uninstall   # Uninstall gateway service
    arba setup               # Interactive setup wizard
    arba logout              # Clear stored authentication
    arba status              # Show status of all components
    arba cron                # Manage cron jobs
    arba cron list           # List cron jobs
    arba cron status         # Check if cron scheduler is running
    arba doctor              # Check configuration and dependencies
    arba honcho setup                    # Configure Honcho AI memory integration
    arba honcho status                   # Show Honcho config and connection status
    arba honcho sessions                 # List directory → session name mappings
    arba honcho map <name>               # Map current directory to a session name
    arba honcho peer                     # Show peer names and dialectic settings
    arba honcho peer --user NAME         # Set user peer name
    arba honcho peer --ai NAME           # Set AI peer name
    arba honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    arba honcho mode                     # Show current memory mode
    arba honcho mode [hybrid|honcho|local]  # Set memory mode
    arba honcho tokens                   # Show token budget settings
    arba honcho tokens --context N       # Set session.context() token cap
    arba honcho tokens --dialectic N     # Set dialectic result char cap
    arba honcho identity                 # Show AI peer identity representation
    arba honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    arba honcho migrate                  # Step-by-step migration guide: OpenClaw native → Arba + Honcho
    arba version             Show version
    arba update              Update to latest version
    arba uninstall           Uninstall Arba
    arba acp                 Run as an ACP server for editor integration
    arba sessions browse     Interactive session picker with search

    arba claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``arba update``
...
# this guard, arba crashes on import and the user can't run
# ``arba update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'arba' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``arba tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``arba.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("arba")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"arba", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"arba")
        # Windows: the .exe name is already ``arba.exe`` — nothing to do.
    except Exception:
        pass
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`arba --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    ...
    print(f"Arba v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``arba --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (arba tools, arba setup, arba model) use
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any arba module import.
#
...
def _apply_profile_override() -> None:
    ...
    # /opt/data/profiles/coder).  If HERMES_HOME points to the arba root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `arba profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # 2. If no flag, check active_profile in the arba root
    ...
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent arba from starting
            print(
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Arba itself has been explicitly configured (model
    ...
    # Only count these if Arba has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Arba to use their tokens.
...
def _print_tui_exit_summary(
    ...
    print(f"  arba --tui --resume {target}")
    if title:
        print(f'  arba --tui -c "{title}"')
    print()
...
def _launch_tui(
    ...
    # in the user's shell would otherwise make a plain `arba --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `arba update` so
...
def _pin_kanban_board_env() -> None:
    ...
    (``arba kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``arba kanban boards switch`` from another session can flip the file
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Arba — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Arba still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'arba sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'arba doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Arba isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  arba setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'arba setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Arba?")
        print()
    ...
    # has no creds.json.  Every subsequent `arba gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: arba gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `arba gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  arba gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Arba'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  arba gateway")
        ...
        print()
        print("  Or install as a service: arba gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'arba whatsapp' to try again.")
...
def cmd_postinstall(args):
    ...
    print("⚕ Arba post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``arba model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'arba model' for "
                "available providers, or run 'arba doctor' to diagnose config "
                "issues."
            )
...
#
# Arba uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `arba model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `arba model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Arba only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``arba model`` flow, then route aux tasks to them here.
    """
    from hermes_cli.config import load_config
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # bad paste short of re-running `arba setup` from scratch. OpenRouter
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `arba auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``arba model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``arba model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``arba auth add xai-oauth``).  Fall
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `arba model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Arba URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  Arba will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Arba will install it now (the preflight below "
...
# fast paths like `arba --version` and slash-command dispatch that never
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Arba turns to `copilot --acp`.")
    print("  Arba currently starts its own ACP subprocess for each request.")
    print("  Arba uses your selected model as a hint for the Copilot ACP session.")
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``arba setup`` / ``arba model``.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `arba setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so arba knows where to find it
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Arba typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Arba, enable billing on your "
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Arba will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'arba model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Arba CLI with a provider."""
...
def cmd_slack(args):
    ...
    Dispatches ``arba slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: arba slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `arba slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_backup(args):
    """Back up Arba home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Arba backup from a zip file."""
...
# even run ``arba update`` again to roll forward. The post-pull syntax
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``arba update --gateway`` so interactive prompts (stash restore,
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``arba update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``arba web``).
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (arba web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``arba update`` can unconditionally call ``arba desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``arba desktop`` (interactive launch) skips the build when the
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``arba update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Arba.app/Contents/MacOS/Arba"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Arba.exe",
            release_dir / "win-ia32-unpacked" / "Arba.exe",
            release_dir / "win-arm64-unpacked" / "Arba.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "hermes",
            release_dir / "linux-unpacked" / "Arba",
        ]
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Arba is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Arba.app/Contents/MacOS/Arba  ->  app bundle = .../Arba.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Arba Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Arba Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  arba gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Arba is
    ...
    if source_mode:
        print("→ Launching Arba Desktop from source build...")
    ...
    print(f"→ Launching packaged Arba Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``arba dashboard`` processes other than ourselves.

    ``arba dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``arba update`` replaces files on disk, the running
    ...
    patterns = [
        "arba dashboard",
        "hermes dashboard",  # legacy command alias
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `arba update`.
    ...
    print("  Preview now:  arba curator run --dry-run")
    print("  Pause it:     arba curator pause")
    print(
        "  Docs:         https://arcadiaglobalteknologi.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``arba update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``arba update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: arba curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``arba dashboard`` processes.

    Called at the end of ``arba update`` (default ``reason``) and also
    from ``arba dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    arba dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Arba by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `arba update "
            f"--branch {branch}`, or update against main with `arba update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/arcadiaglobalteknologi/arba/archive/refs/heads/{branch}.zip"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Arba behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes arba completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Arba couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Arba couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Arba behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `arba update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/arcadiaglobalteknologi/arba.git",
    "git@github.com:arcadiaglobalteknologi/arba.git",
    "https://github.com/arcadiaglobalteknologi/arba",
    "git@github.com:arcadiaglobalteknologi/arba",
}
OFFICIAL_REPO_URL = "https://github.com/arcadiaglobalteknologi/arba.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Arba repository.")
        print("  This means you may miss updates from arcadiaglobalteknologi/arba.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/arcadiaglobalteknologi/arba.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/arcadiaglobalteknologi/arba.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``arba update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``arba update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    The Arba Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    so during ``arba update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``arba update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``arba update``
    ...
    #      hermes.exe sitting *under* a non-Arba parent (e.g. a Arba
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Arba Desktop, exit any open `hermes` REPLs, and")
    lines.append("  stop the gateway (`arba gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `arba update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``arba update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    the next arba invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Arba Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Arba Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Arba Desktop, exit other `hermes` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `arba update`."
        )
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    Called early on every arba invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``arba update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `arba update` once the upstream issue is resolved.")
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `arba update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``arba update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``arba update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``arba update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``arba update --gateway``) the update is already
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``arba update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``arba update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``arba update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# Arba — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``arba update``
    ...
    print(f"  Restore:  arba import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``arba update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Arba to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `arba update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Arba via pip (for PyPI installs)."""
    ...
    print("✓ Update complete! Restart arba to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # every user who ran ``arba update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``arba update`` again later once a fix lands.")
                    else:
        ...
        # build.  ``arba desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``arba desktop`` shouldn't be forced into a full
        # Electron build by ``arba update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            ...
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `arba desktop` to retry)")
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'arba config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: arba config migrate")
            else:
                print()
                print("Skipped. Run 'arba config migrate' later to configure.")
        else:
        ...
        # check `arba curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``arba update`` gives users a predictable
        ...
        # When running as ``arba update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: arba gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: arba -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Arba gateway unit files are still installed.
        # When both hermes.service (from a pre-rename install) and the
        # current hermes-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `arba update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Arba gateway unit(s) detected:")
                ...
                print()
                print("    arba gateway migrate-legacy")
                print()
        ...
        print("  arba model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``arba -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `arba profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → arba -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  arba profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     arba -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `arba profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  arba -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      arba -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `arba profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"arba -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Arba {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Arba {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``arba dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``arba dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No arba dashboard processes running.")
        return 0

    print(f"{len(pids)} arba dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No arba dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `arba update`.
    ...
    # the same logs directory as every other Arba surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
...
def cmd_logs(args):
    """View and filter Arba log files."""
...
# so that in ``arba -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``arba -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `arba --tui` is the hot path on phones. The full parser setup imports
...
def main():
    ...
    # ``arba update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://arcadiaglobalteknologi.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `arba model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://arcadiaglobalteknologi.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        ...
        description=(
            "Stop, disable, and remove legacy Arba gateway unit files "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `arba proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Arba with an interactive wizard. "
        "Run a specific section: arba setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'arba setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Arba.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Arba")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Arba CLI",
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Arba with Spotify via PKCE"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Arba components",
    )
    ...
    cron_create.add_argument(
        "--profile",
        help="Arba profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--profile",
        help="Arba profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Arba setup",
    )
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Arba "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Arba Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Arba setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Arba. Use 'arba debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    arba debug share              Upload debug report and print URL
    arba debug share --lines 500  Include more log lines
    arba debug share --expire 30  Keep paste for 30 days
    arba debug share --local      Print report locally (no upload)
    arba debug share --no-redact  Disable upload-time secret redaction
    arba debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'arba debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Arba home directory to a zip file",
        description="Create a zip archive of your entire Arba configuration, "
    ...
    checkpoints_parser = subparsers.add_parser(
        ...
        "repo arba uses to snapshot working directories before "
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Arba backup from a zip file",
        description="Extract a previously created Arba backup into your "
        "Arba home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Arba configuration",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "so future 'arba update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`arba update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `arba update`. Pass --sync to re-seed now."
        ),
    )
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `arba plugins enable <name>`",
    )
    ...
    # subcommand — ``arba --help``, ``arba version``, ``arba logs``,
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'arba tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # arba tools disable <name...> [--platform cli]
    ...
    # arba tools enable <name...> [--platform cli]
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `arba computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `arba tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: arba computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: arba computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Arba as an MCP server",
        description=(
            "Manage MCP server connections and run Arba as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'arba mcp add' to connect to a new server, or\n"
            "'arba mcp serve' to expose Arba conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Arba as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `arba mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `arba mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `arba mcp install n8n`)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch arba --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Arba",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Arba",
    ...
    claw_migrate.add_argument(
        ...
        "before apply; restorable with 'arba import').",
    )
    claw_migrate.add_argument(
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Arba to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'arba config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, arba will "
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Arba",
        description="Remove Arba from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Arba as an ACP (Agent Client Protocol) server",
        description="Start Arba in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Arba ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Arba provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Arba instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `arba update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `arba profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Arba profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Arba web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `arba --tui` via PTY/WebSocket). "
    ...
    # for `arba dashboard` cmdlines and SIGTERM them directly — the same
    # path `arba update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running arba dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running arba dashboard processes and exit",
    )
    ...
    # for one release. The Arba-Setup.exe success screen tells users to
    # run `arba desktop` from a terminal, so the canonical name needs
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Arba Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--hermes-root",
        help="Override the Arba source root used by Desktop (sets HERMES_DESKTOP_HERMES_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Arba log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    arba logs                    Show last 50 lines of agent.log
    arba logs -f                 Follow agent.log in real time
    arba logs errors             Show last 50 lines of errors.log
    arba logs gateway -n 100     Show last 100 lines of gateway.log
    arba logs gui -f             Follow gui.log in real time
    arba logs --level WARNING    Only show WARNING and above
    arba logs --session abc123   Filter by session ID
    arba logs --component tools  Only show tool-related lines
    arba logs --since 1h         Lines from the last hour
    arba logs --since 30m -f     Follow, starting from 30 min ago
    arba logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``arba -c Pokemon Agent Dev`` → ``arba -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'arba -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (arba hooks list, cron

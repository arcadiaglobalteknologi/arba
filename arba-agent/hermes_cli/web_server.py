...
try:
    ...
    # running `arba dashboard` needs fastapi+uvicorn; lazy install keeps
...
app = FastAPI(title="Arba", version=__version__, lifespan=_lifespan)
...
# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``arba dashboard --tui``
...
async def get_status():
    ...
    # and which providers are registered so ``arba status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Arcadia Global Teknologi" or
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the arba home root).
    """
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``arba <subcommand>`` detached and record the Popen handle.
...
async def restart_gateway():
    """Kick off a ``arba gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``arba update`` in the background."""
    ...
    try:
        ...
        _log.exception("Failed to spawn arba update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Arba supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `arba model` does so GUI onboarding lands on a
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Arba from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Arba to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Arba from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Arba to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Arba in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Arba through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Arba via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Arba through an IMAP/SMTP mailbox.",
        "docs_url": "https://arcadiaglobalteknologi.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Arba to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Arba inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Arba through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Arba to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Arba to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Arba as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://arcadiaglobalteknologi.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://arcadiaglobalteknologi.com/docs/user-guide/messaging/webhooks/",
...
# the canonical ``arba auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Arba resolves Anthropic creds in this order at runtime:
    1. ``~/.hermes/.anthropic_oauth.json`` — Arba-managed PKCE flow
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Arba PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Arba even
    when they also have a separate Arba-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "arba auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "arba auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "arba auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "arba auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "arba auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "arba auth add xai-oauth",
        "docs_url": "https://arcadiaglobalteknologi.com/docs/guides/xai-grok-oauth",
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Arba-managed PKCE file
...
# Guarded so arba web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Arba file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``arba auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
# the tokens exactly like `arba auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `arba auth add xai-oauth`'s credential-pool insert.
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `arba sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `arba sessions prune`)."""
...
# servers managed here show up under `arba mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog():
    ...
    `arba mcp catalog` / `arba mcp install` read.
    """
    try:
...
# spawn the real `arba gateway <verb>` so behaviour matches the CLI exactly.
...
# `arba memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `arba memory setup` to configure a new one.",
            )
...
def _profile_setup_command(name: str) -> str:
    ...
    return "arba setup" if name == "default" else f"{name} setup"
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``arba tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``arba tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``arba tools``
...
#
# The endpoint spawns the same ``arba --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``arba --tui`` would run.  Tests monkeypatch this
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Arba inside WSL2 to use the dashboard's /chat "
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the Arba backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Arba Teal",         "description": "Classic dark teal — the canonical Arba look"},
    {"name": "default-large", "label": "Arba Teal (Large)", "description": "Arba Teal with bigger fonts and roomier spacing"},
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Arba exposes a
    ...
    print(f"  Arba Web UI → http://{host}:{port}")

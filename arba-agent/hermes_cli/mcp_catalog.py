...
entries via ``arba mcp catalog`` or the interactive ``arba mcp picker``,
and install them with ``arba mcp install <name>`` (or by toggling in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``arba mcp install <name>`` to
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Arba install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Arba understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Arba is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Arba
        understands. Update Arba to install this entry.
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``arba mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `arba mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `arba mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `arba mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `arba mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `arba auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`arba auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Arba session to load its tools.",
        Colors.GREEN,
    ))

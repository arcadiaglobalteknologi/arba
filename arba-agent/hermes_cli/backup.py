...
`arba backup` creates a zip archive of the entire ~/.hermes/ directory
(excluding the hermes-agent repo and transient files).

`arba import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to arba root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Arba home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Arba home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    print(f"\nRestore with: arba import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Arba backup.
    ...
    # Look for telltale files that a arba home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Arba backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a arba dir name
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Arba configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: arba profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "hermes-agent").is_dir():
            print("Note: The hermes-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: arba update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  arba -p {pname} gateway install")

        print("Done. Your Arba configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and arba backup --quick)
# ---------------------------------------------------------------------------
...
# — `arba update` snapshots this set before pulling so approved-user lists
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``arba update``.
    ...
    Args:
        ...
        hermes_home: Override for the Arba home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for arba backup --quick."""
...
def create_pre_update_backup(
    ...
    (``arba update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `arba claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `arba import` and the

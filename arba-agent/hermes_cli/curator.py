"""CLI subcommand: `arba curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `arba curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`arba curator status` and run `arba curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`arba curator unpin {args.skill}`"
        )
        return 1
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`arba curator backup` or wait for the next curator run."
            )
        else:
            print(
...
def cli_main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="arba curator")
    register_cli(parser)

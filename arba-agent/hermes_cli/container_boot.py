...
def reconcile_profile_gateways(
    *,
    ...
    is what ``arba gateway start`` (no ``-p``) targets. Without it,
    bare ``arba gateway start`` inside the container would land on
    ...
    # ``arba gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `arba profile create` (config.yaml
            # is not — that comes later via `arba setup`). Use it as the
            ...
            # via ``arba -p default-named gateway start`` if they
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name in ("arba", "hermes"):
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `arba -p <profile> gateway start`
        ...
        # (all dispatched as the arba user) won't hit EACCES. See

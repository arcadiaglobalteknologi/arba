"""arba webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    arba webhook subscribe <name> [options]
    arba webhook list
...
def _setup_hint() -> str:
  ...
  Then start the gateway: arba gateway run
"""
...
def webhook_command(args):
    """Entry point for 'arba webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: arba webhook {subscribe|list|remove|test}")
        print("Run 'arba webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (arba gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: arba webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from arba webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (arba gateway run)")

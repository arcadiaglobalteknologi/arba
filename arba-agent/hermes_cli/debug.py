"""``arba debug`` debug tools for Arba.

Currently supports:
    arba debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[arba debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``arba debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``arba debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Arba version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `arba debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``arba debug share`` invocation added ~20 MB of resident Python
    ...
    ``arba debug share`` also runs an opportunistic sweep as a fallback
...
def _capture_dump() -> str:
    """Run ``arba dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``arba dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "arba debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  arba debug delete <url>")

    print(f"\nShare these links with the Arba team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: arba debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'arba debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``arba debug`` call.
    ...
    # best-effort — any failure is swallowed so ``arba debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: arba debug <command>")
        print()
        print("Commands:")

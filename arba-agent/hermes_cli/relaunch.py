"""
Unified self-relaunch for Arba CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``arba sessions browse`` or post-setup relaunch
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh arba invocation.
    ...
    relaunch contract wants: "run arba again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    That's functionally equivalent — the user sees "arba exited, then
    new arba started" — just with two PIDs in play instead of one.
    """

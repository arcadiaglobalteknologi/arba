"""
Interactive setup wizard for Arba.
...
_DOCS_BASE = "https://arcadiaglobalteknologi.com/docs"
...
# display_hermes_home imported lazily at call sites (stale-module safety during arba update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Arba Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Arba using environment variables or config commands:")
    print_info("  arba config set model.provider custom")
    print_info("  arba config set model.base_url http://localhost:8080/v1")
    print_info("  arba config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'arba setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'arba setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'arba setup' to configure"))
    ...
    # Video generation — opt-in via `arba tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'arba setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'arba setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'arba setup terminal'"))
    ...
    # Spotify (OAuth via arba auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'arba setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('arba setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('arba setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('arba setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('arba setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('arba setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('arba config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('arba config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('arba config set <key> <value>', Colors.GREEN)}")
    ...
    print(f"   {color('arba gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('arba doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `arba tools` and `arba setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``arba model``)
    ...
    provider added to ``arba model`` is automatically available here.
    ...
    # Delegate to the shared arba model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: arba model")
    ...
    # on demand via `arba auth add`, `arba setup` vision, and
    # `arba setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``arba model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Arba auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via arba setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'arba setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Arba runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `arba setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `arba setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `arba setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Arba delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `arba update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://arcadiaglobalteknologi.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Arba",
            bot_description="Your Arba agent on Slack",
        )
        ...
        print_info(
            "   Re-run `arba slack manifest --write` anytime to refresh after "
            "Arba adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Arba delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Arba to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://arcadiaglobalteknologi.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://arcadiaglobalteknologi.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  arba config edit")
    print_info("   Open config in your editor:  arba config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Arba from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'arba setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     arba config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo arba gateway install --system")
                print_info("  Or run in foreground:  arba gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   arba gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   arba gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `arba setup tools` and `arba tools` use the same flow:
...
# Config values may have different semantics between OpenClaw and Arba.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Arba to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Arba at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Arba at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Arba at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Arba at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Arba equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Arba config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Arba.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Arba's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Arba can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: arba claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: arba claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Arba configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Arba config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Arba (use arba claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``arba setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``arba setup --portal``)
    that gets a brand-new user from zero to a fully working Arba session
    ...
    print(color("│          ⚕ Arba Setup — Nous Portal (one-shot)          │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `arba setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `arba auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `arba auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `arba model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `arba portal status` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      arba setup           — full or quick (auto-detected)
      arba setup model     — just model/provider
      arba setup tts       — just text-to-speech
      arba setup terminal  — just terminal backend
      arba setup gateway   — just messaging platforms
      arba setup tools     — just tool configuration
      arba setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│       ⚕ Arba Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│                  ⚕ Arba Setup Wizard                   │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Arba installation.               │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Arba configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'arba setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Arba?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'arba config edit'")
    ...
    # Tune later with `arba setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``arba setup <section>``
    or switch providers with ``arba model``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'arba setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    arba setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  arba setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'arba setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Arba to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'arba setup gateway'.")

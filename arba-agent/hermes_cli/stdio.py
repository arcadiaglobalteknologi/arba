...
on Japanese locales, etc.).  Arba's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Arba is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Arba-managed Git install directories so
    ...
    # install without Arba-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Arba's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Arba's
    ``arba config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Arba (User env var in Windows Settings, or
    export in a PowerShell profile) and Arba picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Arba-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Arba spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Arba-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Arba venv Scripts directory — host of the hermes.exe shim itself,

...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Arba bash completion
# Add to ~/.bashrc:
#   eval "$(arba completion bash)"
...
complete -F _hermes_completion arba hermes
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef arba hermes
# Arba zsh completion
# Add to ~/.zshrc:
#   eval "$(arba completion zsh)"
...
{top_cmds_str}
            ...
            _describe 'arba command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Arba fish completion",
        "# Add to your config:",
        "#   arba completion fish | source",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c arba -f -s p -l profile"

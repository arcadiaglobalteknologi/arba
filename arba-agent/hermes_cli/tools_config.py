"""
Unified tool configuration for Arba.

`arba tools` and `arba setup tools` both enter this module.
...
# who want it opt in via `arba tools` → Video Generation, which walks
...
# `arba tools` → X (Twitter) Search setup walks users through credential
...
# Platform-scoped toolsets: only appear in the `arba tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``arba tools`` → "reconfigure existing" would
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Arba routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``arba update`` and
      by ``arba computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `arba update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Arba process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/arcadiaglobalteknologi/arba:latest"
            )
            return
        ...
        # Run the full `arba auth spotify` flow — if the user has no
        ...
        try:
            ...
            _print_info("    Run manually: arba auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `arba auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: arba auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: arba plugins enable observability/langfuse")
        _print_info("    Restart Arba for tracing to take effect.")
        _print_info("    Verify: arba plugins list")
        ...
        try:
            ...
            _print_info("    Run later: arba auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `arba auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: arba auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: arba auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # through ``arba tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `arba tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `arba tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `arba tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by arba tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `arba tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `arba tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `arba model`).
    if managed_feature:
        from hermes_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `arba tools` and `arba setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Arba Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://arcadiaglobalteknologi.com/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, arba mcp

"""Helpers for loading Arba .env files consistently across entrypoints."""
...
# Used by setup / `arba model` flows to label detected credentials so
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `arba setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    already knows all valid Arba env-var names and can split
...
def load_hermes_dotenv(
    *,
    ...
    """Load Arba environment files with user config taking precedence.
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Arba reads
    ...
    (tests, future ``arba secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `arba model`

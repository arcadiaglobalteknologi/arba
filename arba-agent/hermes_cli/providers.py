"""
Single source of truth for provider identity in Arba.
...
2. **Arba overlays** — transport type, auth patterns, aggregator flags,
...
# -- Arba overlay ----------------------------------------------------------
# Arba-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Arba-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Arba overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Arba overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + arba extra
    ...
    if overlay is not None:
        # Arba-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,

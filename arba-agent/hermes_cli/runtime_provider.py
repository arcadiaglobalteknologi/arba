...
_VALID_API_MODES = {
    ...
    # instead of Arba' tool dispatch. Gated behind config key
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'arba model' or "
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `arba auth list`).  If the key is
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Arba canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Arba-compatible importers).

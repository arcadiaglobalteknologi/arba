"""``arba plugins`` CLI subcommand — install, update, remove, and list plugins.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Arba subprocess resolution: :func:`shutil.which` first,
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Arba.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `arba plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Arba plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `arba plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  arba gateway restart[/dim]")
    console.print()
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``arba plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] arba plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] arba plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] arba plugins")
    console.print("[dim]Enable/disable:[/dim] arba plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Arba
...
def plugins_command(args) -> None:
    """Dispatch arba plugins subcommands."""

"""``arba portal`` — small CLI surface for Nous Portal users.
...
already in ``arba auth`` or ``arba tools``. It's a discovery + status
...
DOCS_URL = "https://arcadiaglobalteknologi.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   arba auth add nous --type oauth")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `arba model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `arba auth add nous --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `arba portal <subcommand>`."""
    ...
    print("Run `arba portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `arba portal` on the given argparse subparsers object."""

"""
Security advisory checker for Arba.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``arba doctor --ack <id>``; the ack is persisted to
...
1. ``arba doctor`` (and ``arba doctor --ack <id>``)
...
run in environments where the rest of Arba failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including arba when configured "
        ...
        remediation=(
            ...
            "After cleanup: arba doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Arba requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'arba doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``arba doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `arba doctor` on the gateway host for details.")

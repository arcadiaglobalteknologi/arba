...
docker run -it --rm \
  -v ~/.hermes:/opt/data \
  arcadiaglobalteknologi/arba setup
```
...
docker run -d \
  ...
  -v ~/.hermes:/opt/data \
  -p 8642:8642 \
  arcadiaglobalteknologi/arba gateway run
```
...
services:
  hermes-work:
    image: arcadiaglobalteknologi/arba:latest
...
FROM arcadiaglobalteknologi/arba:latest
...
When a tool must be available immediately on every container start with no re-install delay, build a new image that inherits from `arcadiaglobalteknologi/arba` and installs the tool in a layer:
...
The entrypoint script and `/opt/data` semantics are inherited unchanged, so the rest of this page still applies. Remember to rebuild the image when pulling a newer upstream `arcadiaglobalteknologi/arba`.
...
If a tool is likely to be useful to most Hermes Agent users, consider contributing it upstream rather than carrying it in a private derived image. Open an issue or pull request on the [hermes-agent repository](https://github.com/arcadiaglobalteknologi/arba) describing the tool and its use case. Tools that get bundled into the official image benefit every user and avoid the maintenance overhead of a downstream fork.
...
docker run -it --rm arcadiaglobalteknologi/arba:latest version     # Verify version

...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Arba silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Arba CLI where sys.stdout and sys.stderr both report
...
def _emergency_cleanup_all_sessions():
    ...
    crashed arba processes — this way every clean arba exit sweeps
    ...
    # Sweep orphans from other crashed arba processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live arba processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    ...
    orphan reaper can distinguish daemons owned by a live arba process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning arba process is no longer alive.
    ...
    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         ...
         cross-process safe: two concurrent arba instances won't reap each
         other's daemons.
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live arba process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Arba-managed
    ...
    # Build an extended search PATH including Arba-managed Node, macOS
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Arba
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/arcadiaglobalteknologi/arba:latest")
                else:

...
def _get_active_profile_name() -> str:
    """Return the active Arba profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Arba process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=hermes-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a arba process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Arba
    process leaves its container behind permanently because there's no
    subsequent Arba process scheduled to reuse that exact (task, profile)
    pair.
    """
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # _sanitize_label_value(); the active Arba profile is captured at
        ...
        # container shared across sessions").  If a prior Arba process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Arba secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        # container) but can happen if a previous Arba process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Arba exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Arba process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Arba startup. That covers the
        ...
        # outlives Arba processes, processes inside it stay alive, and

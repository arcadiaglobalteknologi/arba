"""
Lazy dependency installer for opt-in Arba backends.

Many Arba features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
remediation hint pointing at ``arba tools`` or the manual pip command.
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``arba update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``arba update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``arba update``. Never raises; lazy-install failures

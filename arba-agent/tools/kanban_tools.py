...
orchestrator work. A normal ``arba chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``arba kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``arba kanban complete …``
...
Humans continue to use the CLI (``arba kanban …``), the dashboard
(``arba dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``arba chat`` without the kanban toolset see zero
...
def _connect(board: Optional[str] = None):
    ...
    the env-pinned active board without restarting Arba.
    """

...
Backend is selected during ``arba tools`` setup (web.backend in config.yaml).
When available, Arba can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``arba tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `arba tools` repaint.
        try:

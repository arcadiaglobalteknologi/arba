...
def read_suppressed_names() -> Set[str]:
    ...
    what makes pruning a built-in durable: without it, ``arba update`` would
...
def list_agent_created_skill_names() -> List[str]:
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip Arba metadata, VCS, virtualenv/dependency, and cache dirs
...
def list_archived_skill_names() -> List[str]:
    ...
    so the directory name is the skill name. Used by ``arba curator
    list-archived`` to help users pass a name to ``arba curator restore``.
    """

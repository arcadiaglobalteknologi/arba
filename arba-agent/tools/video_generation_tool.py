...
provider** — turn on a backend by enabling a plugin (``arba plugins
enable video_gen/<name>``) and selecting it in ``arba tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `arba tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `arba plugins list` to see "
            f"installed video gen backends, or `arba tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `arba tools` → "
...
# `arba tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`arba tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `arba tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}

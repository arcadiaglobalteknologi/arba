...
# Marker file written by `arba profile create --no-skills` (named profiles)
...
# installer, `arba update`, nor a direct sync re-injects bundled skills.
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # this skip, every `arba update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `arba skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `arba skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `arba update` runs "
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    installer, ``arba update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``arba skills opt-out`` /
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `arba update` (or `arba skills "

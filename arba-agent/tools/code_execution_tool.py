...
Lets the LLM write a Python script that calls Arba tools via RPC,
...
# available on every platform Arba itself runs on.
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Arba-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Arba-profile soft guard."""',
...
"""Auto-generated Arba tools RPC stubs."""
...
"""Auto-generated Arba tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    try:
        ...
        # HERMES_TIMEZONE is an internal Arba setting and must not leak
...
def _load_config() -> dict:
    ...
    ``arba --tui`` where it is never used.  Read the lightweight raw config
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``arba tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call Arba tools programmatically. "

# Contributing to Arba

Thank you for contributing to Arba! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Arba to work everywhere.
...
Bundled skills (in `skills/`) ship with every Arba install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `arba skills browse` (labeled "official") and install it with `arba skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Arcadia Global Teknologi Discord](https://arcadiaglobalteknologi.com). Users can install it with `arba skills install`.
...
- Integrate with `arba memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/arcadiaglobalteknologi/arba.git
cd hermes-agent
...
├── tests/                    # Test suite
├── website/                  # Documentation site (arcadiaglobalteknologi.com)
│
...
The user may skip setup and keep loading the skill. Arba only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `arba setup` or update `~/.hermes/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Arba" is the secondary collaborator. If the contributor's commit shows "Arba" as author because they used Arba to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Arba tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `arba --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Arba uses a data-driven skin system — no code changes needed to add a new skin.
...
Arba runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the arba wrapper (it has a stdlib fallback
...
Arba has terminal access. Security matters.
...
2. **Test manually**: Run `arba` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/arcadiaglobalteknologi/arba/issues)
- Include: OS, Python version, Arba version (`arba version`), full error traceback
...
- **Discord**: [arcadiaglobalteknologi.com](https://arcadiaglobalteknologi.com) — for questions, showcasing projects, and sharing skills

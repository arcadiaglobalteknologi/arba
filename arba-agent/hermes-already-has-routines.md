# Arba Has Had "Routines" Since March
...
Arba equivalent — works today:
```bash
arba cron create "0 2 * * *" \
...
Arba equivalent — works today:
```bash
arba webhook subscribe auth-watch \
  --events "pull_request" \
...
Arba equivalent — works today:
```bash
arba webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Arba implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Arba |
|---|---|---|
...
## Things Arba Does That Routines Can't
...
Arba has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Arba is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
arba gateway setup    # enable webhooks
arba webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [arcadiaglobalteknologi.com/docs/guides/automation-templates](https://arcadiaglobalteknologi.com/docs/guides/automation-templates)

Documentation: [arcadiaglobalteknologi.com](https://arcadiaglobalteknologi.com)

GitHub: [github.com/arcadiaglobalteknologi/arba](https://github.com/arcadiaglobalteknologi/arba)

---

*Arba is built by [Arcadia Global Teknologi](https://arcadiaglobalteknologi.com). Open source, model-agnostic, runs on your infrastructure.*

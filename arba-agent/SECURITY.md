# Arba Security Policy

This document describes Arba's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/arcadiaglobalteknologi/arba/security/advisories/new)
or **security@arcadiaglobalteknologi.com**. Do not open public issues for
security vulnerabilities. **Arba does not operate a bug
bounty program.**
...
- Environment details (`arba version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Arba access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Arba's documentation
...
Arba supports two OS-level isolation postures. They address
...
Arba supports this in two ways:

- **Arba's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Arba's in-process heuristics
...
Arba filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Arba
itself. Bugs in Arba's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Arba does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Arba's own documentation, or reasonable
  operator expectations would predict — including cases where
  Arba has documented a stance about how its output should
...
- Arba includes supply-chain guards for MCP server
...
- **Channel:** the GHSA thread or email correspondence with
  security@arcadiaglobalteknologi.com.

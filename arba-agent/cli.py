#!/usr/bin/env python3
"""
Arba CLI - Interactive Terminal Interface

A beautiful command-line interface for the Arba, inspired by Claude Code.
...
try:
    ...
    # yet — happens during partial ``arba update`` where git-reset landed
...
def load_cli_config() -> Dict[str, Any]:
    ...
    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``arba chat --ignore-user-config``),
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Arba, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Arba - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``arba --help`` paths inside the chat command flow).  See
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run arba -w")
        return None
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    These are auto-generated by ``arba -w`` sessions and PR review
...
#   6. Default: assume dark (matches the legacy Arba assumption)
#
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Arba output it's common to display cron schedules verbatim.
    #
...
# ASCII Art - ARBA logo (full width, single line)
...
# ASCII Art - Arba Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ ARBA - AI Agent Framework"
        tiny_line = "⚕ ARBA"
    else:
        agent_name = _skin.get_branding("agent_name", "Arba") if _skin else "Arba"
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Arba v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Arcadia Global Teknologi[/]\n"
...
class HermesCLI:
    ...
    def __init__(
        ...
        # by `arba chat --ignore-rules` in hermes_cli/main.py. When true we
        ...
        # it's shared across all Arba processes for this HERMES_HOME.
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Arba'}"
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from hermes_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Arba")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: arba setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: arba setup")
            return False
        ...
        # `arba chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `arba auth add openai-codex`
        # without `arba model`), fall back to the provider's first catalog
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (arba sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (arba sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``arba doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``arba doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Arba needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Arba: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Arba: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: arba --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Arba config/state.
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run arba chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'arba setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Arba!{_RST}")
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `arba gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `arba sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `arba sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        ...
        DB, so the live session remains resumable via ``arba --resume <id>``
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: arba --resume {self.session_id}")
        except Exception as e:
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `arba model` and gateway pickers).
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Arba default (chat_completions)
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to hermes_cli.curator so the CLI and the `arba curator`
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``arba bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: arba bundles create "
        ...
        _cprint(
            ...
            f"Manage with `arba bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Arba-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Arba keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Arba is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Arba is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Arba is busy."
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Arba to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``arba update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Arba')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Arba"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Arba",
            detail="This will exit the current session and run `arba update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  arba --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  arba -c \"{session_title}\"{profile_flag}")
            print()
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Arba! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Arba! Type your message or /help for commands."
        ...
        # after an OpenClaw→Arba migration (especially migrations done by
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Arba binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Arba")
        ...
        # Dynamic prompt: shows Arba symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Arba needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Arba needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Arba needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the arba process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: arba setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Arba CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Arba Gateway (messaging platforms)...")
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``arba chat -q "work kanban task <id>"``;
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Arba" box, no tool-gen
            ...
            # Single-query mode (`arba chat -q "…"`): skip the welcome

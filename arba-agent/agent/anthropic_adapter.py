"""Anthropic Messages API adapter for Arba.

Translates between Arba's internal OpenAI-style message format and
...
# Arba effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Arba historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Arba)
    ...
    # 1. Arba-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Arba configs.
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Arba-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Arba with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Arba-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Arba OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Arba", "Claude Code")
                text = text.replace("Arba agent", "Claude Code")
    ...
    # silently hides reasoning text that Arba surfaces in its CLI. We

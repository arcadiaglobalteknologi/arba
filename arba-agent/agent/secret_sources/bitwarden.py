...
Arba pulls API keys from Bitwarden Secrets Manager at process startup
...
* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  Arba pins one version (``_BWS_VERSION``) and downloads
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  hammer the API.
* Failures NEVER block Arba startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `arba chat -q ...`
...
def _hermes_bin_dir() -> Path:
    """Where Arba stores its managed binaries.  Profile-aware."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``arba secrets bitwarden setup``
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `arba secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `arba secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `arba secrets bitwarden setup` to install."
        )
        return result

...
appropriate.  All installs go to a Arba-owned bin staging dir,
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``arba lsp
  status``.
...
def hermes_lsp_bin_dir() -> Path:
    """Return the Arba-owned bin staging dir for LSP servers."""
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``arba lsp status`` CLI to give users a quick

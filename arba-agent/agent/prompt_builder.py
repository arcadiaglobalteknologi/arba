...
DEFAULT_AGENT_IDENTITY = (
    "You are Arba, an intelligent AI assistant created by Arcadia Global Teknologi "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Arba "
    "itself, load the `hermes-agent` skill with skill_view(name='hermes-agent') "
    "before answering. Docs: https://arcadiaglobalteknologi.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `arba kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Arba WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Arba itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Arba runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Arba itself is running. The host OS, home, and cwd "
                f"of the Arba process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Arba "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Arba
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Arba itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `hermes-agent` skill "
            "first. It has the actual commands (e.g. `arba config set …`, `arba tools`, "
            "`arba setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: arba setup, arba setup tools, arba setup terminal, arba status.",
        ]
    )
    return "\n".join(lines)

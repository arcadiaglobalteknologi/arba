...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Arba with shell-template placeholders. **Wins over a
...
``{success: False, error: …}`` JSON envelope the rest of Arba
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``arba tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``arba setup`` for
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``arba tools`` picker.

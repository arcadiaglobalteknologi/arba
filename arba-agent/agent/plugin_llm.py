...
Plugins built on Arba often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Arba
...
embedded slugs, no shorthands. This mirrors Arba' main config
...
handles every provider, fallback chain, and per-task override Arba
supports.
"""

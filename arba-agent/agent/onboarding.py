...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Arba starts and finds ``~/.openclaw/``.

    Points users at ``arba claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``arba claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Arba, run "
        "`arba claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `arba claw cleanup` (renames it to ~/.openclaw.pre-migration — "

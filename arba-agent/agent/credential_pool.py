...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``arba model`` / ``arba auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Arba process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (e.g. setup wizard, ``arba auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Arba
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Arba process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Arba profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `arba model` / `arba auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `arba model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``arba auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `arba auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `arba auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Arba PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `arba setup` ("Claude
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `arba auth add nous --label <name>`).  Fall back to the
        ...
        # Arba auth store or env vars, so resolve them here.
        ...
        # standard `arba auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `arba auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Arba auth store.  Without this gate the removal is instantly
        ...
        # Arba owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``arba model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``arba auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # authoritative source for Arba credentials. Stale env vars from parent
    ...
    # Honour user suppression — `arba auth remove <provider> <N>` for an
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # Arba PKCE is Arba-owned/persistable while present, but it is

...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `arba update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``arba update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``arba curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `arba curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Arba' background skill CURATOR. This is an "
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every arba install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: arba curator status
        keep an umbrella stable: arba curator pin document-tools
    ...
    lines.append("full report: arba curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: arba curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "safety and can be restored via `arba curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Restore any via `arba curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `arba curator restore <name>`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `arba curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `arba update` only if "
        ...
        # recorded in state so `arba curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``arba model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `arba model` → auxiliary

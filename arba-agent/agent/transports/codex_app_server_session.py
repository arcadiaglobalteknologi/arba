...
Owns one Codex thread per Arba session. Drives `turn/start`, consumes
...
# Arba' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Arba/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Arba session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Arba' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Arba' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Arba' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Arba approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Arba returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort Arba version string for codex's userAgent line."""
    try:
